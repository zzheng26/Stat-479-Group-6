...
weather = read_csv("project_weather_data in year resolution.csv")%>%
  select(city, year, EMXT) %>%
  merge(y = geo.ref, all.x = TRUE, by = "city")
```


```{r}
# Cleanig population data
...
full_data = left_join(weather, homeless, by = c("year", "State")) %>%
  ...
  mutate("Percent Homeless" = `Overall Homeless` / Population)

write.csv(full_data, "homeless_weather.csv", row.names = FALSE)
```
